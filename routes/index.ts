...
function randomArticle(seed: number): string {
  let articles: string[] = ['a', 'the'];
...
function randomAjective(seed: number): string {
  var generator = new MersenneTwister(seed);
  let adjectives: string[] = ['hushed','teeny-tiny','decisive','long-term','alcoholic',
...
function aToAnIfNeeded(sentence: string): string {
...
function sentenceF(seed: number): string {
  var generator = new MersenneTwister(seed);
  sentences = [sentenceA, sentenceB, sentenceC, sentenceD];
  return sentences[Math.floor(generator.random() * sentences.length)](seed++) +
...
function sentenceG(seed: number): string {
  var generator = new MersenneTwister(seed);
  sentences = [sentenceA, sentenceB, sentenceC, sentenceD];
  return sentences[Math.floor(generator.random() * sentences.length)](seed++) +
...
function sentenceH(seed: number): string {
  var generator = new MersenneTwister(seed);
  sentences = [sentenceA, sentenceB, sentenceC, sentenceD];
  return sentences[Math.floor(generator.random() * sentences.length)](seed++) +
...
function sentenceI(seed: number): string {
  var generator = new MersenneTwister(seed);
  sentences = [sentenceA, sentenceB, sentenceC, sentenceD];
  return sentences[Math.floor(generator.random() * sentences.length)](seed++) +
...
function sentenceJ(seed: number): string {
  var generator = new MersenneTwister(seed);
  sentences = [sentenceA, sentenceB, sentenceC, sentenceD];
  return sentences[Math.floor(generator.random() * sentences.length)](seed++) +
...
function sentenceK(seed: number): string {
  var generator = new MersenneTwister(seed);
  sentences = [sentenceA, sentenceB, sentenceC, sentenceD];
  return sentences[Math.floor(generator.random() * sentences.length)](seed++) +
...
function sentenceL(seed: number): string {
  var generator = new MersenneTwister(seed);
  sentences = [sentenceA, sentenceB, sentenceC, sentenceD];
  return sentences[Math.floor(generator.random() * sentences.length)](seed++) +
...
function sentenceM(seed: number): string {
  var generator = new MersenneTwister(seed);
  sentences = [sentenceA, sentenceB, sentenceC, sentenceD];
  return sentences[Math.floor(generator.random() * sentences.length)](seed++) +
...
function randomSentence(seed: number): string {
  ...
  let sentence = sentences[Math.floor(generator.random() * sentences.length)](seed++);
...
function randomLink(seed: number, hostname: string): object {
  let href: string = '/' + randomSentence(seed).replace(/ /g, '/').replace(/,/g, '')
  return {
    href,
    title: randomTitle(generateSeed(hostname + href))
  }
}

function randomLinks(seed: number, hostname: string): object[] {
  let generator = new MersenneTwister(seed+10000);
  let linkCount = 5 + Math.floor(generator.random() * 10);
  let links: object[] = [];
...
function generateSeed(path: string): number {
  let md5 = require('md5');
  let sum = md5(path);
  let seed = parseInt(sum.slice(0,7),16) + parseInt(sum.slice(8,15),16) + parseInt(sum.slice(16,23),16) + parseInt(sum.slice(24,31),16);
  return seed;
}
...
function randomPage(req: Request, res: Response) {
  ...
  let links = randomLinks(seed, req.hostname);

  res.render('random', {title: title, paragraphs: paragraphs, links: links});
}

indexRouter.all('*', randomPage);
